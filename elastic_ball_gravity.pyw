...
def print_energy():
    global Elast, Eclock_d, delta_E, displayE, Eclock
    E = 0
    for obj in objs:
    ...
    text_surface = my_font.render(
        'Total Energy: '+str(displayE), True, (255, 255, 255))
...
while True:
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            color = (random.randint(20, 255), random.randint(
                20, 255), random.randint(20, 255))
            v = Vec2(10*(random.random()-0.5), 10*(random.random()-0.5))
            objs.append(Ball(10, random.randint(100, 500),
                        Vec2(pos[0], pos[1]), v, color))
    for i in range(nsub):
        update(dt)
    render()
    print_energy()
