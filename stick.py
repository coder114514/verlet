...
def renderSticstiffnesss():
    for i in sticstiffnesss:
        ...
        pygame.draw.line(screen,(255,255,255),(s.p1.x,s.p1.y),(s.p2.x,s.p2.y))
...
def renderSprings():
    for i in springs:
        s=springs[i]
        if s.hidden: continue
        pygame.draw.line(screen,(255,0,0),(s.p1.x,s.p1.y),(s.p2.x,s.p2.y))

def renderRects():
    for i in rects:
        r=rects[i]
        points=[]
        for p in r.points: points.append((p.x,p.y))
...
def renderEngines():
    for i in engines:
        ...
        pygame.draw.line(screen,(255,255,255),(e.x0-e.A,e.y0),(e.x0+e.A,e.y0))
        pygame.draw.line(screen,(255,255,255),(e.x0,e.y0-e.A),(e.x0,e.y0+e.A))
...
while True:
    for event in pygame.event.get():
        ...
        #     buttonDown=True
        #     pygame.mouse.set_pos((engines[1].x,engines[1].y))
        # elif event.type==MOUSEBUTTONUP:
        #     buttonDown=False
        # if event.type==MOUSEMOTION and buttonDown:
        #     (engines[1].x,engines[1].y)=pygame.mouse.get_pos()
    updatePoints()
    updateEngines()
    updateSprings()
    for i in range(3):
